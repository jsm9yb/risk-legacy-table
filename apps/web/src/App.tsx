...
import LanApp from "./net/LanApp.tsx";
...
type View = { kind: "hub" } | { kind: "local"; cfg: LocalConfig } | { kind: "lan" };
...
export default function App() {
  ...
  if (view.kind === "local") return <LocalCampaign config={view.cfg} onExit={() => setView({ kind: "hub" })} />;
  if (view.kind === "lan") return <LanApp onExit={() => setView({ kind: "hub" })} />;
  return <Hub
