...
All ten **faction starting powers** (SPEC §6) are implemented and `confirmed`: the first player to select a faction chooses between its two green powers, that choice attaches to the faction permanently, and every application emits `FactionPowerApplied`. Prepare the World separately places the official 12 starting Resource stickers (maximum 3 resources per Territory card).
...
New worlds now begin with the authoritative **Prepare the World** ritual: seat 3–5 players and alternate twelve individually persisted Coin-sticker placements across all 42 Territory cards before Game 1 can exist. The card surface includes a map reference and explains that Territory cards have unequal resource values. Faction powers are chosen later by the first player to select each faction. Once Pack 1 is open, each game visibly reveals the high-roll order, runs the five-round physical snake draft, requires players to take their drafted Coin cards from the public pile into their private hand, then places factions and starts turns in the independently drafted orders. Both rituals resume after refresh in local hot-seat and LAN play.
